...
<div>
    ...
    <div class="max-w-5xl">
        <form method="POST" action="/prescriptions" id="prescription-form" class="space-y-6">
            @csrf
            ...
            .then(data => {
                if (data.success && data.data.length > 0) {
                    dropdown.innerHTML = data.data.map(p => `
                        <div class="px-4 py-2 hover:bg-slate-100 cursor-pointer" onclick='selectPatient(${JSON.stringify(p).replace(/'/g, "&#39;")})'>
