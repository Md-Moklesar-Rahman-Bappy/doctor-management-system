...
<div>
    ...
    <div class="bg-white rounded-xl border border-slate-200 overflow-hidden">
        <div class="p-4 border-b border-slate-200">
            <form method="GET" action="/doctors" class="flex items-center gap-2">
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tbody class="divide-y divide-slate-200">
                    ...
                    <tr class="hover:bg-slate-50">
                        ...
                        <td class="px-4 py-4">
                            <div class="flex items-center gap-2">
                                <a href="/doctors/{{ $doctor->id }}" class="p-2 text-sky-600 hover:bg-sky-50 rounded-lg" title="View">
                                ...
                                <a href="/doctors/{{ $doctor->id }}/edit" class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg" title="Edit">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                </a>
                                <form action="/doctors/{{ $doctor->id }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="p-2 text-red-600 hover:bg-red-50 rounded-lg" title="Delete" onclick="return confirm('Delete this doctor?')">
                    ...
                    @empty
                    <tr>
                        <td colspan="6" class="px-6 py-8 text-center text-slate-500">No doctors found</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
